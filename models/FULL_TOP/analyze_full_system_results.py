...
# Helper to read full system testbench output file with multiple testcases
def read_full_system_output(filename, num_tests=5, num_classes=15):
    outputs = []
    test_names = []
...
def main():
    parser = argparse.ArgumentParser(description="Analyze and compare full system hardware and software outputs.")
    parser.add_argument('--hw', type=str, default='full_system_outputs.txt', help='Hardware output file from full system testbench')
    parser.add_argument('--sw', type=str, default='sw_full_system_output.mem', help='Software (PyTorch) output .mem file')
    ...
    parser.add_argument('--num_tests', type=int, default=5, help='Number of testcases (default: 5)')
    parser.add_argument('--report', type=str, default='full_system_analysis.png', help='Output plot/report file')
    args = parser.parse_args()

    # Read hardware outputs (multiple testcases)
    if os.path.exists(args.hw):
        hw_outputs, test_names = read_full_system_output(args.hw, args.num_tests, args.num_classes)
        print(f"Loaded {len(hw_outputs)} full system hardware testcases")
    else:
    ...
    if hw_outputs:
        print("\n=== FULL SYSTEM COMPREHENSIVE ANALYSIS ===")
        ...
        for i, (hw_out, test_name) in enumerate(zip(hw_outputs, test_names)):
            ...
            axs[0,0].plot(x, hw_out, marker='o', color=color, label=f'Test {i}: {test_name[:25]}...')
        if len(sw_output) > 0:
            axs[0,0].plot(x, sw_output, marker='s', linewidth=3, color='black', label='Software Reference')
        axs[0,0].set_title('Full System: All Testcase Outputs Comparison')
        ...
        if len(sw_output) > 0:
            ...
            axs[0,1].bar(range(len(errors)), errors, color='orange', alpha=0.7)
            axs[0,1].set_title('Full System: Mean Absolute Error per Testcase')
        ...
        axs[1,0].bar(x, output_ranges, color='green', alpha=0.7)
        axs[1,0].set_title('Full System: Output Range per Class (across all testcases)')
        ...
        axs[1,1].bar(range(len(means)), means, yerr=stds, capsize=5, color='purple', alpha=0.7)
        axs[1,1].set_title('Full System: Output Statistics per Testcase')
        ...
        print(f"\nFull system analysis plot saved to {args.report}")

        # Print detailed statistics
        print("\n=== FULL SYSTEM DETAILED STATISTICS ===")
        for i, (hw_out, test_name) in enumerate(zip(hw_outputs, test_names)):
            print(f"\nTest {i}: {test_name}")
        ...
        print(f"Total testcases analyzed: {len(hw_outputs)}")
        print(f"Classes per output: {args.num_classes}")
        print(f"Bit width: {args.width}")
        
        # Calculate overall statistics
        all_values = np.concatenate(hw_outputs)
        print(f"Overall statistics across all testcases:")
        print(f"  Global mean: {np.mean(all_values):.2f}")
        print(f"  Global std:  {np.std(all_values):.2f}")
        print(f"  Global min:  {np.min(all_values)}")
        print(f"  Global max:  {np.max(all_values)}")
